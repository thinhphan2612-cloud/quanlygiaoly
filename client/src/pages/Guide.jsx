...
const GUIDE = [
  {
    id: 'batdau', emoji: '🚀', title: 'Bắt đầu nhanh', roles: ['admin'],
    ...
    steps: [
      ...
      'Xong phần khung, mỗi buổi học chỉ cần: Điểm danh → nhập Điểm số → cuối năm Lưu trữ & lên lớp.',
    ],
    tips: ['Nên hoàn tất Cài đặt (tên + logo) trước khi xuất chứng chỉ/giấy khen, vì các mẫu này lấy dữ liệu từ đó.'],
  },
  {
    id: 'tongquan', emoji: '🏠', title: 'Tổng quan (Dashboard)', roles: ['admin', 'teacher'],
  ...
  {
    id: 'hocvien', emoji: '🧒', title: 'Học viên', roles: ['admin', 'teacher'],
  ...
  {
    id: 'lophoc', emoji: '📚', title: 'Lớp học', roles: ['admin', 'teacher'],
  ...
  {
    id: 'giaolyvien', emoji: '👩‍🏫', title: 'Giáo lý viên', roles: ['admin'], pro: true,
    ...
    steps: [
      ...
      'Xếp GLV vào một hoặc nhiều lớp. Từ đó họ đăng nhập và chỉ thấy/были thao tác trên lớp được phân.',
  ...
  {
    id: 'diemdanh', emoji: '✅', title: 'Điểm danh', roles: ['admin', 'teacher'],
  ...
  {
    id: 'diemso', emoji: '📊', title: 'Điểm số', roles: ['admin', 'teacher'],
  ...
  {
    id: 'dethi', emoji: '📝', title: 'Đề thi & Thi online', roles: ['admin', 'teacher'], pro: true,
  ...
  {
    id: 'chungchi', emoji: '🎖️', title: 'Chứng chỉ & Giấy khen', roles: ['admin'], pro: true,
  ...
  {
    id: 'trabai', emoji: '🎲', title: 'Chọn trả bài (bốc thăm)', roles: ['admin', 'teacher'],
  ...
  {
    id: 'game', emoji: '🎮', title: 'Game học giáo lý', roles: ['admin', 'teacher'],
  ...
  {
    id: 'store', emoji: '🛍️', title: 'Ephata Store', roles: ['admin', 'teacher'],
  ...
  {
    id: 'thuchi', emoji: '💰', title: 'Thu chi', roles: ['admin'], pro: true,
  ...
  {
    id: 'luutru', emoji: '🗄️', title: 'Lưu trữ niên khóa & Lên lớp', roles: ['admin'], pro: true,
  ...
  {
    id: 'thongbao', emoji: '🔔', title: 'Thông báo & Chuông báo vắng', roles: ['admin'], pro: true,
  ...
  {
    id: 'caidat', emoji: '⚙️', title: 'Cài đặt quản lý', roles: ['admin'],
  ...
  {
    id: 'goi', emoji: '✦', title: 'Gói dịch vụ & Nâng cấp', roles: ['admin'],
...
export default function Guide() {
  ...
  const jump = (id) => document.getElementById('gd-' + id)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      ...
      <div className="guide-body">
        <aside className="guide-toc">
          ...
          {(nq ? filtered : GUIDE).map((s) => (
            <button key={s.id} onClick={() => jump(s.id)}>{s.emoji} {s.title}</button>
          ))}
          ...
          {filtered.map((s) => (
            <section className="gd-card" id={'gd-' + s.id} key={s.id}>
              <div className="gd-card-head">
                <span className="gd-emoji">{s.emoji}</span>
                <h2>{s.title}</h2>
              ...
              <p className="gd-summary">{s.summary}</p>
              {s.steps?.length > 0 && (
                <>
                  <div className="gd-sub">Quy trình</div>
                  <ol className="gd-steps">{s.steps.map((t, i) => <li key={i}>{t}</li>)}</ol>
                </>
              )}
              {s.tips?.map((t, i) => (
                <div className="gd-tip" key={i}><b>Mẹo:</b> {t}</div>
              ))}
            </section>
          ))}
